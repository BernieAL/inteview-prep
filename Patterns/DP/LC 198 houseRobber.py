...
we only need to maintain last 2 houses we can rob
iterate through each of the houses we can rob
    compute max we can rob up to ith house in temp var
    temp = max(N+rob1,rob2)

    #[rob1,rob2,n,n+1,...]
     to take rob1, you have to take n too
     to take rob2, you cant include n

     then update rob1 to rob2 
     then update rob2 to n

"""
def rob(nums):
    rob2,rob1,curmax = 0,0,0

    for i in nums:
        curmax = max(rob1,i+rob2)
        rob2 = rob1
        rob1 = curmax
    return curmax